...
import { List, InputItem, Button, WhiteSpace, WingBlank, ImagePicker } from 'antd-mobile';
...
class Set extends React.Component {
    ...
    onChange = async (files, type, index) => {
        ...
        const {url} = files[0];
        console.log(url);
        let {portraitUrl}  = await request.post('/users/portrait', {
            portrait: url
        })
        console.log('请求回来的',data)
        this.setState({
            files,
        });
    }
    ...
    render() {
        ...
        />
                ...
                <Button onClick={() => this.props.history.push('/mine')} style={{ marginBottom: '80px'}}>返回</Button>
            </div>
        )
    }
}
